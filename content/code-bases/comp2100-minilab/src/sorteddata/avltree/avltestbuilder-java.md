...
## Path
src/sorteddata/avltree/AVLTestBuilder.java

## Explanation

This test file defines the AVLTestBuilder class in the sorteddata.avltree package. It belongs to src/sorteddata/avltree in the COMP2100 MiniLab codebase and verifies behavior of the avl test builder implementation. Key methods include makeWithCast, make, empty, setTreeRoot.
...
```mermaid
classDiagram
class AVLTestBuilder {
  <<test>>
  -comparator: Comparator<T>
  -tree: AVLTree<T>
  +AVLTestBuilder(comparator: Comparator<T>)
  +makeWithCast(value: T, left: Object, right: Object) AVLNode<T>
  +make(value: T, left: AVLNode<T>, right: AVLNode<T>) AVLNode<T>
  +make(value: T) AVLNode<T>
  +empty() AVLNode<T>
  +setTreeRoot(root: AVLNode<T>) void
  +setTreeRootWithCast(root: Object) void
  +getTree() AVLTree<T>
}
```
