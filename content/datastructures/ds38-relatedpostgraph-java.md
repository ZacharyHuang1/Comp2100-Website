...
DS38_RelatedPostGraph is a Mock_hackathon practice implementation for DS38: Related-post graph. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Connect posts with shared tags, links, or cross-references. The task is: Undirected graph adjacency list.
...
PlantUML guidance:
DS38_RelatedPostGraph ..> Post : reads post id/topic
DS38_RelatedPostGraph ..> Message : reads message id/text/timestamp
DS38_RelatedPostGraph ..> User : reads user id/username

## UML

```mermaid
classDiagram
class DS38_RelatedPostGraph {
...
class Message
class User
DS38_RelatedPostGraph ..> Post : accepts model object
DS38_RelatedPostGraph ..> Message : accepts model object
DS38_RelatedPostGraph ..> User : accepts model object
```
...
/**
 * DS38 practice implementation for related-post graph.
 */
public class DS38_RelatedPostGraph {
    private final Map<UUID, Set<UUID>> adjacency = new LinkedHashMap<>();

    // Creates an empty directed graph.
    public DS38_RelatedPostGraph() {
    }
