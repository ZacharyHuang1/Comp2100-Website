...
DS47_TimestampRangeSearchService is a Mock_hackathon practice implementation for DS47: Timestamp range search service. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Find posts/messages in a time interval. The task is: AVL range traversal or TreeMap sub-map idea.
...
PlantUML guidance:
DS47_TimestampRangeSearchService ..> Post : reads post id/topic
DS47_TimestampRangeSearchService ..> Message : reads message id/text/timestamp
DS47_TimestampRangeSearchService ..> User : reads user id/username
DS47_TimestampRangeSearchService ..> SortedData : returns snapshot
DS47_TimestampRangeSearchService ..> SortedDataFactory : builds factory-backed snapshot
DS47_TimestampRangeSearchService ..> AVLTree : exposes AVL snapshot
DS47_TimestampRangeSearchService ..> BSTree : exposes BST snapshot
DS47_TimestampRangeSearchService ..> SortedArrayList : exposes array snapshot

## UML

```mermaid
classDiagram
class DS47_TimestampRangeSearchService {
...
class Message
class User
DS47_TimestampRangeSearchService ..> Post : accepts model object
DS47_TimestampRangeSearchService ..> Message : accepts model object
DS47_TimestampRangeSearchService ..> User : accepts model object
class SortedData
...
DS47_TimestampRangeSearchService ..> SortedData : returns snapshot
DS47_TimestampRangeSearchService ..> SortedDataFactory : builds snapshot
DS47_TimestampRangeSearchService ..> AVLTree : builds AVL snapshot
DS47_TimestampRangeSearchService ..> BSTree : builds BST snapshot
DS47_TimestampRangeSearchService ..> SortedArrayList : builds array snapshot
```
...
/**
 * DS47 practice implementation for timestamp range search service.
 */
public class DS47_TimestampRangeSearchService {
    private final TreeMap<Long, Set<UUID>> values = new TreeMap<>();

    // Creates an empty range index.
    public DS47_TimestampRangeSearchService() {
    }
