...
DS36_TopKWordCounter is a Mock_hackathon practice implementation for DS36: Top-K word counter. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Find most frequent words in posts/messages. The task is: HashMap counts plus heap for top K.
...
PlantUML guidance:
DS36_TopKWordCounter ..> Post : reads post id/topic
DS36_TopKWordCounter ..> Message : reads message id/text/timestamp
DS36_TopKWordCounter ..> User : reads user id/username

## UML

```mermaid
classDiagram
class DS36_TopKWordCounter {
...
class Entry
DS36_TopKWordCounter *-- Entry
class Post
class Message
class User
DS36_TopKWordCounter ..> Post : accepts model object
DS36_TopKWordCounter ..> Message : accepts model object
DS36_TopKWordCounter ..> User : accepts model object
```
...
/**
 * DS36 practice implementation for top-K word counter.
 */
public class DS36_TopKWordCounter {
    private final PriorityQueue<Entry> queue = new PriorityQueue<>();
    private final Map<UUID, Entry> entries = new HashMap<>();
    private long sequence;

    // Creates an empty priority queue.
    public DS36_TopKWordCounter() {
    }
