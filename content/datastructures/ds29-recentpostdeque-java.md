...
DS29_RecentPostDeque is a Mock_hackathon practice implementation for DS29: Recent post deque. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Maintain bounded recent activity list. The task is: Deque of recent post ids with capacity.
...
PlantUML guidance:
DS29_RecentPostDeque ..> Post : reads post id/topic
DS29_RecentPostDeque ..> Message : reads message id/text/timestamp
DS29_RecentPostDeque ..> User : reads user id/username

## UML

```mermaid
classDiagram
class DS29_RecentPostDeque {
...
class Message
class User
DS29_RecentPostDeque ..> Post : accepts model object
DS29_RecentPostDeque ..> Message : accepts model object
DS29_RecentPostDeque ..> User : accepts model object
```
...
/**
 * DS29 practice implementation for recent post deque.
 */
public class DS29_RecentPostDeque {
