...
DS17_MutualFollowRecommender is a Mock_hackathon practice implementation for DS17: Mutual follow recommender. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Suggest users based on follow graph. The task is: Compute candidates followed by people I follow.
...
PlantUML guidance:
DS17_MutualFollowRecommender ..> Post : reads post id/topic
DS17_MutualFollowRecommender ..> Message : reads message id/text/timestamp
DS17_MutualFollowRecommender ..> User : reads user id/username

## UML

```mermaid
classDiagram
class DS17_MutualFollowRecommender {
...
class Message
class User
DS17_MutualFollowRecommender ..> Post : accepts model object
DS17_MutualFollowRecommender ..> Message : accepts model object
DS17_MutualFollowRecommender ..> User : accepts model object
```
