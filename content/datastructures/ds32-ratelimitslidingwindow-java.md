...
DS32_RateLimitSlidingWindow is a Mock_hackathon practice implementation for DS32: Rate-limit sliding window. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Prevent spam by limiting actions per time window. The task is: Queue timestamps per user; remove expired timestamps.
...
PlantUML guidance:
DS32_RateLimitSlidingWindow ..> Post : reads post id/topic
DS32_RateLimitSlidingWindow ..> Message : reads message id/text/timestamp
DS32_RateLimitSlidingWindow ..> User : reads user id/username

## UML

```mermaid
classDiagram
class DS32_RateLimitSlidingWindow {
...
class Entry
DS32_RateLimitSlidingWindow *-- Entry
class Post
class Message
class User
DS32_RateLimitSlidingWindow ..> Post : accepts model object
DS32_RateLimitSlidingWindow ..> Message : accepts model object
DS32_RateLimitSlidingWindow ..> User : accepts model object
```
