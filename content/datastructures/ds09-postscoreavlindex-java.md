...
DS09_PostScoreAVLIndex is a Mock_hackathon practice implementation for DS09: Post score AVL index. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Maintain posts sorted by votes/replies/activity score. The task is: Remove and reinsert when a score changes to preserve ordering.
...
PlantUML guidance:
DS09_PostScoreAVLIndex ..> Post : reads post id/topic
DS09_PostScoreAVLIndex ..> Message : reads message id/text/timestamp
DS09_PostScoreAVLIndex ..> User : reads user id/username
DS09_PostScoreAVLIndex ..> SortedData : returns snapshot
DS09_PostScoreAVLIndex ..> SortedDataFactory : builds factory-backed snapshot
DS09_PostScoreAVLIndex ..> AVLTree : exposes AVL snapshot
DS09_PostScoreAVLIndex ..> BSTree : exposes BST snapshot
DS09_PostScoreAVLIndex ..> SortedArrayList : exposes array snapshot

## UML

```mermaid
classDiagram
class DS09_PostScoreAVLIndex {
...
class Message
class User
DS09_PostScoreAVLIndex ..> Post : accepts model object
DS09_PostScoreAVLIndex ..> Message : accepts model object
DS09_PostScoreAVLIndex ..> User : accepts model object
class SortedData
...
DS09_PostScoreAVLIndex ..> SortedData : returns snapshot
DS09_PostScoreAVLIndex ..> SortedDataFactory : builds snapshot
DS09_PostScoreAVLIndex ..> AVLTree : builds AVL snapshot
DS09_PostScoreAVLIndex ..> BSTree : builds BST snapshot
DS09_PostScoreAVLIndex ..> SortedArrayList : builds array snapshot
```
...
/**
 * DS09 practice implementation for post score AVL index.
 */
public class DS09_PostScoreAVLIndex {
    private final TreeMap<Long, Set<UUID>> values = new TreeMap<>();

    // Creates an empty range index.
    public DS09_PostScoreAVLIndex() {
    }
