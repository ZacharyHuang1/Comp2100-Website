...
DS05_UserToPostIndex is a Mock_hackathon practice implementation for DS05: User-to-post index. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Show all posts written by one user on a profile page. The task is: Maintain a mapping from userId to the user’s posts.
...
PlantUML guidance:
DS05_UserToPostIndex ..> Post : reads post id/topic
DS05_UserToPostIndex ..> Message : reads message id/text/timestamp
DS05_UserToPostIndex ..> User : reads user id/username

## UML

```mermaid
classDiagram
class DS05_UserToPostIndex {
...
class Message
class User
DS05_UserToPostIndex ..> Post : accepts model object
DS05_UserToPostIndex ..> Message : accepts model object
DS05_UserToPostIndex ..> User : accepts model object
```
...
/**
 * DS05 practice implementation for user-to-post index.
 */
public class DS05_UserToPostIndex {
