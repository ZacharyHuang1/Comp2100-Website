...
DS44_DraftPostStore is a Mock_hackathon practice implementation for DS44: Draft post store. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Store unsubmitted drafts per user. The task is: Map user to draft ids, sorted by update time.
...
PlantUML guidance:
DS44_DraftPostStore ..> Post : reads post id/topic
DS44_DraftPostStore ..> Message : reads message id/text/timestamp
DS44_DraftPostStore ..> User : reads user id/username
DS44_DraftPostStore ..> SortedData : returns snapshot
DS44_DraftPostStore ..> SortedDataFactory : builds factory-backed snapshot
DS44_DraftPostStore ..> AVLTree : exposes AVL snapshot
DS44_DraftPostStore ..> BSTree : exposes BST snapshot
DS44_DraftPostStore ..> SortedArrayList : exposes array snapshot

## UML

```mermaid
classDiagram
class DS44_DraftPostStore {
...
class Message
class User
DS44_DraftPostStore ..> Post : accepts model object
DS44_DraftPostStore ..> Message : accepts model object
DS44_DraftPostStore ..> User : accepts model object
class SortedData
...
DS44_DraftPostStore ..> SortedData : returns snapshot
DS44_DraftPostStore ..> SortedDataFactory : builds snapshot
DS44_DraftPostStore ..> AVLTree : builds AVL snapshot
DS44_DraftPostStore ..> BSTree : builds BST snapshot
DS44_DraftPostStore ..> SortedArrayList : builds array snapshot
```
...
/**
 * DS44 practice implementation for draft post store.
 */
public class DS44_DraftPostStore {
    private final TreeMap<Long, Set<UUID>> values = new TreeMap<>();

    // Creates an empty range index.
    public DS44_DraftPostStore() {
    }
