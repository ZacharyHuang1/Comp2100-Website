...
DS14_SavedPostsIndex is a Mock_hackathon practice implementation for DS14: Saved posts index. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Let users bookmark posts. The task is: Map each user to a set of saved post ids.
...
PlantUML guidance:
DS14_SavedPostsIndex ..> Post : reads post id/topic
DS14_SavedPostsIndex ..> Message : reads message id/text/timestamp
DS14_SavedPostsIndex ..> User : reads user id/username

## UML

```mermaid
classDiagram
class DS14_SavedPostsIndex {
...
class Message
class User
DS14_SavedPostsIndex ..> Post : accepts model object
DS14_SavedPostsIndex ..> Message : accepts model object
DS14_SavedPostsIndex ..> User : accepts model object
```
...
/**
 * DS14 practice implementation for saved posts index.
 */
public class DS14_SavedPostsIndex {
