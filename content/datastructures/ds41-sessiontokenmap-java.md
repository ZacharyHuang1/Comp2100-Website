...
DS41_SessionTokenMap is a Mock_hackathon practice implementation for DS41: Session token map. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Map login tokens to users with expiry. The task is: HashMap token -> session and priority queue by expiry.
...
PlantUML guidance:
DS41_SessionTokenMap ..> Post : reads post id/topic
DS41_SessionTokenMap ..> Message : reads message id/text/timestamp
DS41_SessionTokenMap ..> User : reads user id/username

## UML

```mermaid
classDiagram
class DS41_SessionTokenMap {
...
class Entry
DS41_SessionTokenMap *-- Entry
class Post
class Message
class User
DS41_SessionTokenMap ..> Post : accepts model object
DS41_SessionTokenMap ..> Message : accepts model object
DS41_SessionTokenMap ..> User : accepts model object
```
...
/**
 * DS41 practice implementation for session token map.
 */
public class DS41_SessionTokenMap {
    private final PriorityQueue<Entry> queue = new PriorityQueue<>();
    private final Map<UUID, Entry> entries = new HashMap<>();
    private long sequence;

    // Creates an empty priority queue.
    public DS41_SessionTokenMap() {
    }
