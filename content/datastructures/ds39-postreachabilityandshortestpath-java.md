...
DS39_PostReachabilityAndShortestPath is a Mock_hackathon practice implementation for DS39: Post reachability and shortest path. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Find connected discussions through related-post graph. The task is: BFS/DFS with visited set.
...
PlantUML guidance:
DS39_PostReachabilityAndShortestPath ..> Post : reads post id/topic
DS39_PostReachabilityAndShortestPath ..> Message : reads message id/text/timestamp
DS39_PostReachabilityAndShortestPath ..> User : reads user id/username

## UML

```mermaid
classDiagram
class DS39_PostReachabilityAndShortestPath {
...
class Message
class User
DS39_PostReachabilityAndShortestPath ..> Post : accepts model object
DS39_PostReachabilityAndShortestPath ..> Message : accepts model object
DS39_PostReachabilityAndShortestPath ..> User : accepts model object
```
...
/**
 * DS39 practice implementation for post reachability and shortest path.
 */
public class DS39_PostReachabilityAndShortestPath {
    private final Map<UUID, Set<UUID>> adjacency = new LinkedHashMap<>();

    // Creates an empty directed graph.
    public DS39_PostReachabilityAndShortestPath() {
    }
