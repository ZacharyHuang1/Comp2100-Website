...
DS37_FeedMergeIterator is a Mock_hackathon practice implementation for DS37: Feed merge iterator. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Merge several sorted feeds into one global feed. The task is: Priority queue of iterator heads.
...
PlantUML guidance:
DS37_FeedMergeIterator ..> Post : reads post id/topic
DS37_FeedMergeIterator ..> Message : reads message id/text/timestamp
DS37_FeedMergeIterator ..> User : reads user id/username
...
class Entry
DS37_FeedMergeIterator *-- Entry
class Post
class Message
class User
DS37_FeedMergeIterator ..> Post : accepts model object
DS37_FeedMergeIterator ..> Message : accepts model object
DS37_FeedMergeIterator ..> User : accepts model object
```
