...
DS20_UnreadNotificationQueue is a Mock_hackathon practice implementation for DS20: Unread notification queue. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Queue unread notifications per user. The task is: FIFO queues per user.
...
PlantUML guidance:
DS20_UnreadNotificationQueue ..> Post : reads post id/topic
DS20_UnreadNotificationQueue ..> Message : reads message id/text/timestamp
DS20_UnreadNotificationQueue ..> User : reads user id/username

## UML

```mermaid
classDiagram
class DS20_UnreadNotificationQueue {
...
class Entry
DS20_UnreadNotificationQueue *-- Entry
class Post
class Message
class User
DS20_UnreadNotificationQueue ..> Post : accepts model object
DS20_UnreadNotificationQueue ..> Message : accepts model object
DS20_UnreadNotificationQueue ..> User : accepts model object
```
...
/**
 * DS20 practice implementation for unread notification queue.
 */
public class DS20_UnreadNotificationQueue {
    private final PriorityQueue<Entry> queue = new PriorityQueue<>();
    private final Map<UUID, Entry> entries = new HashMap<>();
    private long sequence;

    // Creates an empty priority queue.
    public DS20_UnreadNotificationQueue() {
    }
