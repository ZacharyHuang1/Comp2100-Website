...
DS21_PriorityNotificationQueue is a Mock_hackathon practice implementation for DS21: Priority notification queue. It is stored separately from the original MiniLab packages so it can be studied as an extension-style hackathon task without changing the base codebase.

The feature is: Prioritize critical notifications over normal ones. The task is: Higher priority first; timestamp tie-breaker.
...
PlantUML guidance:
DS21_PriorityNotificationQueue ..> Post : reads post id/topic
DS21_PriorityNotificationQueue ..> Message : reads message id/text/timestamp
DS21_PriorityNotificationQueue ..> User : reads user id/username

## UML

```mermaid
classDiagram
class DS21_PriorityNotificationQueue {
...
class Entry
DS21_PriorityNotificationQueue *-- Entry
class Post
class Message
class User
DS21_PriorityNotificationQueue ..> Post : accepts model object
DS21_PriorityNotificationQueue ..> Message : accepts model object
DS21_PriorityNotificationQueue ..> User : accepts model object
```
...
/**
 * DS21 practice implementation for priority notification queue.
 */
public class DS21_PriorityNotificationQueue {
    private final PriorityQueue<Entry> queue = new PriorityQueue<>();
    private final Map<UUID, Entry> entries = new HashMap<>();
    private long sequence;

    // Creates an empty priority queue.
    public DS21_PriorityNotificationQueue() {
    }
